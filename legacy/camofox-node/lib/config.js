...
function loadConfig() {
  return {
    ...
    sharedDisplay: process.env.CAMOFOX_SHARED_DISPLAY || '',
    sharedDisplayUserIds: (process.env.CAMOFOX_SHARED_DISPLAY_USER_IDS || '')
      .split(',')
      .map((value) => value.trim())
      .filter(Boolean),
    prometheusEnabled: process.env.PROMETHEUS_ENABLED === '1' || process.env.PROMETHEUS_ENABLED === 'true',
    challengeResolution: {
      mode: process.env.CAMOFOX_CHALLENGE_RESOLUTION_MODE || 'manual_vnc',
      allowlist: (process.env.CAMOFOX_CHALLENGE_ALLOWLIST || '')
        .split(',')
        .map((value) => value.trim())
        .filter(Boolean),
    },
    ...
    serverEnv: {
      ...
      CAMOFOX_SHARED_DISPLAY: process.env.CAMOFOX_SHARED_DISPLAY,
      CAMOFOX_SHARED_DISPLAY_USER_IDS: process.env.CAMOFOX_SHARED_DISPLAY_USER_IDS,
